    ...
    return (
        <div className="space-y-6 sm:space-y-10 animate-in fade-in duration-700 pb-20">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black text-slate-900 tracking-tight">Welcome Back!</h1>
                    <p className="text-slate-500 mt-2 sm:mt-3 text-base sm:text-lg font-medium">Manage your active services and billing details.</p>
                </div>
                <div className="flex flex-wrap gap-3 sm:gap-4">
                    <Link
                        href="/client/websites"
                        className="flex items-center gap-3 bg-slate-900 text-white px-6 sm:px-8 py-4 sm:py-5 rounded-[18px] sm:rounded-[24px] font-black uppercase tracking-widest hover:bg-orange-600 transition-all shadow-xl shadow-slate-900/20 text-[9px] sm:text-[10px]"
                    >
                        <ShoppingBag size={18} />
                        Order website
                    </Link>
                    <a
                        ...
                        className="flex items-center gap-3 bg-white border border-slate-100 text-slate-700 px-6 sm:px-8 py-4 sm:py-5 rounded-[18px] sm:rounded-[24px] font-black uppercase tracking-widest hover:text-orange-600 hover:border-orange-200 transition-all shadow-sm text-[9px] sm:text-[10px]"
                    >
                    ...
                    <Link
                        href="/client/support"
                        className="flex items-center gap-3 orange-gradient text-white px-6 sm:px-8 py-4 sm:py-5 rounded-[18px] sm:rounded-[24px] font-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-2xl shadow-orange-500/30 text-[9px] sm:text-[10px]"
                    >
                        Get Support
                    </Link>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-10">
                ...
                <div className="lg:col-span-2 space-y-6 sm:space-y-8">
                    <h2 className="text-xl sm:text-2xl font-black text-slate-900 flex items-center gap-3 sm:gap-4 uppercase tracking-tight">
                        <div className="w-9 h-9 sm:w-10 sm:h-10 bg-orange-50 text-orange-500 rounded-xl flex items-center justify-center">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-8">
                        {stats?.projects?.length > 0 ? stats.projects.map((project: any) => {
                            ...
                            return (
                                <div key={project.id} className="bg-white p-6 sm:p-8 rounded-[24px] sm:rounded-[40px] border border-slate-100 shadow-sm group hover:border-orange-500/30 transition-all relative overflow-hidden">
                                    ...
                                    <div className="flex justify-between items-start mb-6 sm:mb-8 relative z-10">
                                        <div className="w-12 h-12 sm:w-14 sm:h-14 bg-slate-50 text-orange-600 rounded-2xl flex items-center justify-center group-hover:bg-orange-600 group-hover:text-white transition-all duration-500">
                                            {isHosting ? <Globe size={24} className="sm:size-[28px]" /> : <Server size={24} className="sm:size-[28px]" />}
                                        </div>
                                        <span className="px-3 sm:px-4 py-1.5 sm:py-2 bg-blue-50 text-blue-600 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em]">{project.status}</span>
                                    </div>

                                    <h3 className="text-xl sm:text-2xl font-black text-slate-900 mb-2 relative z-10">{project.name}</h3>
                                    <div className="flex items-center gap-2 mb-4 sm:mb-6 relative z-10">
                                        <div className="px-2.5 sm:px-3 py-1 bg-orange-50 text-orange-600 rounded-lg text-[8px] sm:text-[9px] font-black uppercase tracking-widest border border-orange-100">
                                            {project.package || (isHosting ? 'Starter Hosting' : 'Starter Package')}
                                        </div>
                                        <span className="text-slate-400 text-[9px] sm:text-[10px] font-bold italic">Active {isHosting ? 'Hosting' : 'Management'}</span>
                                    </div>

                                    <div className="pt-6 sm:pt-8 border-t border-slate-50 flex justify-between items-center relative z-10">
                                        <div>
                                            <p className="text-[9px] sm:text-[10px] text-slate-400 uppercase font-black tracking-[0.2em] mb-1">Execution</p>
                                            <p className="text-xs sm:text-sm font-black text-slate-900">{project.progress || 0}% Complete</p>
                                        </div>
                                        <button
                                            onClick={() => {
                                            ...
                                            className="bg-slate-900 text-white px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg sm:rounded-xl font-black text-[9px] sm:text-[10px] uppercase tracking-widest hover:bg-orange-600 transition-all shadow-lg shadow-slate-900/10 flex items-center gap-2"
                                        >
                                            Upgrade <ArrowUpRight size={12} className="sm:size-[14px]" />
                                        </button>
                                    </div>
                                </div>
                            );
                        }) : (
                            <div className="md:col-span-2 bg-white p-12 sm:p-20 rounded-[24px] sm:rounded-[48px] border border-slate-100 border-dashed text-center flex flex-col items-center gap-4 sm:gap-6">
                                <Package size={40} className="text-slate-100 sm:size-[48px]" />
                                <p className="text-slate-400 font-black uppercase tracking-widest text-[9px] sm:text-[10px]">No active projects found.</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                {/* Billing Sidebar */}
                <div className="bg-white p-6 sm:p-10 rounded-[24px] sm:rounded-[48px] border border-slate-100 shadow-sm flex flex-col">
                    <h2 className="text-xl sm:text-2xl font-black text-slate-900 mb-6 sm:mb-10 text-center uppercase tracking-tight">Financials</h2>
            ...
            {showUpgradeModal && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-slate-900/80 backdrop-blur-md animate-in fade-in duration-300">
                    <div className="bg-white w-full max-w-5xl h-[90vh] lg:h-[85vh] rounded-[24px] sm:rounded-[48px] lg:rounded-[64px] shadow-2xl relative overflow-hidden flex flex-col lg:flex-row">
                        ...
                        <div className="lg:w-1/3 bg-slate-900 p-6 sm:p-12 flex flex-col text-white">
                            ...
                            <div className="flex-1">
                                <h3 className="text-[10px] sm:text-xs font-black text-slate-500 uppercase tracking-[0.2em] mb-4 sm:mb-6">Current Deployment</h3>
                            ...
                            <div className="mt-6 sm:mt-12 p-4 sm:p-6 bg-orange-500/10 rounded-[20px] sm:rounded-[32px] border border-orange-500/20">
                                <p className="text-[9px] sm:text-[10px] font-black text-orange-500 uppercase tracking-widest mb-1 sm:mb-2 italic">Infrastructure Priority</p>
                                <p className="text-xs sm:text-sm font-bold text-slate-300">Upgrade to Pro for enterprise-grade uptime, DDoS protection, and global CDN delivery.</p>
                            </div>
                        </div>
                        ...
                        <div className="lg:w-2/3 p-6 sm:p-12 overflow-y-auto bg-slate-50/50">
                            <div className="max-w-4xl mx-auto">
                                <h3 className="text-2xl sm:text-4xl font-black text-slate-900 uppercase tracking-tight mb-2">
                                ...
                                <p className="text-slate-500 text-sm sm:text-base font-medium mb-8 sm:mb-12">Select a strategic tier to optimize your digital infrastructure.</p>
                                    ...
                                    {((selectedProject?.name.toLowerCase().includes('hosting') || (selectedProject?.package && selectedProject?.package.toLowerCase().includes('hosting')))
                                        ...
                                        : managementPackages).map((pkg) => {
                                            ...
                                            return (
                                                <div
                                                    key={pkg.name}
                                                    className={`flex flex-col bg-white p-6 sm:p-8 rounded-[32px] sm:rounded-[48px] border-2 transition-all relative group ${isCurrent
                                                        ? 'border-orange-500 shadow-2xl shadow-orange-500/10 lg:scale-105 z-10'
                                                ...
                                                >
                                                    ...
                                                    <div className="mb-6 sm:mb-8">
                                                        <h4 className="text-xs sm:text-sm font-black text-slate-400 uppercase tracking-[0.2em] mb-1 sm:mb-2">{pkg.name}</h4>
                                                        <div className="flex items-baseline gap-1">
                                                            <span className="text-[10px] sm:text-xs font-black text-slate-400">USD</span>
                                                            <span className="text-3xl sm:text-4xl font-black text-slate-900 tracking-tighter">{pkg.price}</span>
                                                            <span className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest">/M</span>
                                                        </div>
                                                    </div>

                                                    <div className="flex-1 space-y-3 sm:space-y-4 mb-8 sm:mb-10">
                                                        {(Array.isArray(pkg.features) ? pkg.features : []).map((f: any, i: number) => (
                                                            <div key={i} className={`flex items-start gap-2.5 sm:gap-3 text-[9px] sm:text-[10px] font-bold ${f.included ? 'text-slate-700' : 'text-slate-300'}`}>
                                                                {f.included ? (
                                                                    <Check size={12} className="text-green-500 mt-0.5 shrink-0" />
                                                                ) : (
                                                                    <Minus size={12} className="text-slate-200 mt-0.5 shrink-0" />
                                                                )}
                                                    ...
                                                    {!isCurrent ? (
                                                        <button
                                                            ...
                                                            className="w-full py-3.5 sm:py-4 bg-slate-900 hover:bg-orange-600 text-white rounded-[18px] sm:rounded-[24px] font-black uppercase tracking-widest text-[8px] sm:text-[9px] transition-all shadow-xl shadow-slate-900/10 flex items-center justify-center gap-2"
                                                        >
                                                            {upgrading ? <Loader2 className="animate-spin" size={14} /> : 'Deploy Service'}
                                                        </button>
                                                    ) : (
                                                        <div className="w-full py-3.5 sm:py-4 bg-orange-50 text-orange-600 rounded-[18px] sm:rounded-[24px] font-black uppercase tracking-widest text-[8px] sm:text-[9px] text-center border border-orange-100 flex items-center justify-center gap-2">
