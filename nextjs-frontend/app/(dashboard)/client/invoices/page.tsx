    ...
    return (
        <div className="space-y-6 sm:space-y-10 animate-in fade-in duration-700 pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black text-slate-900 tracking-tight uppercase">Billing & Invoices</h1>
                    <p className="text-slate-500 mt-2 font-medium text-base sm:text-lg italic">Manage your payments and download historical invoices.</p>
                </div>
                <button 
                    onClick={fetchInvoices}
                    className="flex items-center justify-center gap-3 bg-slate-900 text-white px-6 sm:px-8 py-3.5 sm:py-4 rounded-[18px] sm:rounded-[24px] font-black uppercase tracking-widest text-[9px] sm:text-[10px] hover:bg-slate-800 transition-all self-start md:self-auto"
                >
                    <RefreshCw size={16} />
                    Refresh
                </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
                <div className="bg-white border border-slate-100 rounded-[20px] sm:rounded-[32px] p-6 sm:p-8 shadow-sm">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 sm:mb-3">Total Invoices</p>
                    <p className="text-3xl sm:text-4xl font-black text-slate-900">{invoices.length}</p>
                </div>
                <div className="bg-white border border-orange-100 rounded-[20px] sm:rounded-[32px] p-6 sm:p-8 shadow-sm">
                    <p className="text-[10px] font-black text-orange-400 uppercase tracking-widest mb-2 sm:mb-3">Outstanding Due</p>
                    <p className="text-3xl sm:text-4xl font-black text-orange-600">{Object.keys(dueByCurrency).length > 1 ? 'Mixed' : totalDue.toFixed(2)}</p>
                    <p className="text-[10px] font-black text-orange-400 uppercase tracking-widest mt-2 sm:mt-3 leading-relaxed">
                        {formatCurrencyTotals(dueByCurrency) || 'No pending invoices'}
                    </p>
                </div>
                <div className="bg-white border border-green-100 rounded-[20px] sm:rounded-[32px] p-6 sm:p-8 shadow-sm">
                    <p className="text-[10px] font-black text-green-500 uppercase tracking-widest mb-2 sm:mb-3">Total Paid</p>
                    <p className="text-3xl sm:text-4xl font-black text-green-600">{Object.keys(paidByCurrency).length > 1 ? 'Mixed' : totalPaid.toFixed(2)}</p>
                    <p className="text-[10px] font-black text-green-500 uppercase tracking-widest mt-2 sm:mt-3 leading-relaxed">
            ...
            {/* Invoice Table */}
            <div className="bg-white border border-slate-100 rounded-[24px] sm:rounded-[48px] overflow-hidden shadow-sm">
                <div className="p-6 sm:p-10 border-b border-slate-50 flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-6">
                    <h2 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight uppercase">Transaction History</h2>
                    <div className="relative w-full sm:w-72">
                        ...
                        <input 
                            ...
                            className="bg-slate-50 border border-slate-100 rounded-[18px] py-3 pl-12 pr-6 text-sm text-slate-900 font-bold outline-none focus:ring-4 focus:ring-orange-500/10 focus:bg-white transition-all shadow-sm w-full" 
                        />
                    </div>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50/50">
                                <th className="p-4 sm:p-8 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Invoice #</th>
                                <th className="p-4 sm:p-8 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Description</th>
                                <th className="p-4 sm:p-8 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Amount</th>
                                <th className="p-4 sm:p-8 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Due Date</th>
                                <th className="p-4 sm:p-8 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Status</th>
                                <th className="p-4 sm:p-8 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] text-right">Action</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-50">
                            {filteredInvoices.length > 0 ? filteredInvoices.map((inv) => (
                                <tr key={inv.id} className="hover:bg-slate-50/50 transition-all group">
                                    ...
                                    <td className="p-4 sm:p-8 text-slate-500 font-bold text-sm">
                                        {inv.due_date 
            ...
            {/* PayHere Note */}
            <div className="bg-white border border-slate-100 p-6 sm:p-10 rounded-[24px] sm:rounded-[48px] flex flex-col md:flex-row items-center gap-6 sm:gap-8 shadow-sm">
