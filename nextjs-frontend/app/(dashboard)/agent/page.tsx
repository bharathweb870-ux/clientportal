    ...
    return (
        <div className="space-y-6 sm:space-y-10 animate-in fade-in duration-700 pb-20">
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
                <div>
                    <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black text-slate-900 tracking-tight">Agent Workspace</h1>
                    <p className="text-slate-500 mt-2 text-base sm:text-lg font-medium">Track your clients, projects, and personal earnings.</p>
                </div>
                <div className="flex flex-wrap items-center gap-4">
                    <div className="bg-white border border-slate-100 rounded-[20px] sm:rounded-[24px] p-2 shadow-sm flex items-center gap-2">
                        <button
                            type="button"
                    ...
                    <Link 
                        ...
                        className="flex items-center justify-center gap-3 orange-gradient text-white px-8 sm:px-10 py-4 sm:py-5 rounded-[18px] sm:rounded-[24px] font-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-2xl shadow-orange-500/30 text-[10px] sm:text-xs"
                    >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-10">
                {/* Analytics Chart */}
                <div className="lg:col-span-2 bg-white border border-slate-100 p-6 sm:p-10 rounded-[24px] sm:rounded-[48px] shadow-sm">
                    <div className="flex items-center justify-between mb-6 sm:mb-10">
                        <div>
                            <h2 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">Earnings Curve</h2>
                            <p className="text-xs sm:text-sm text-slate-400 font-bold uppercase tracking-widest mt-1">Performance Overview</p>
                        </div>
                    </div>
                ...
                <div className="bg-white p-6 sm:p-10 rounded-[24px] sm:rounded-[48px] border border-slate-100 shadow-sm flex flex-col">
                    <h2 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight mb-6 sm:mb-10 uppercase text-center">Recent Network</h2>
                    <div className="space-y-4 sm:space-y-6 flex-1">
                        {stats?.recent_clients?.length > 0 ? stats.recent_clients.map((item: any, i: number) => (
                            <div key={i} className="flex items-center justify-between group cursor-pointer hover:bg-slate-50 p-4 sm:p-5 rounded-[18px] sm:rounded-[28px] transition-all border border-transparent hover:border-orange-100">
                                <div className="flex items-center gap-3 sm:gap-4">
                                    <div className="w-12 h-12 sm:w-14 sm:h-14 orange-gradient rounded-[14px] sm:rounded-2xl flex items-center justify-center font-black text-white shadow-xl shadow-orange-500/20 group-hover:scale-110 transition-transform text-sm sm:text-base shrink-0">
                                        {item.full_name[0]}
                                    </div>
                                    <div>
                                        <p className="font-black text-slate-900 text-sm sm:text-base">{item.full_name}</p>
                                        <p className="text-[9px] sm:text-[10px] text-slate-400 font-black uppercase tracking-[0.2em]">{item.company_name || 'Personal Client'}</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest">{new Date(item.created_at).toLocaleDateString()}</p>
                                </div>
                            </div>
                        )) : (
                    ...
                    <Link 
                        href="/agent/clients"
                        className="w-full mt-6 sm:mt-10 py-4 sm:py-5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl sm:rounded-2xl font-black uppercase tracking-widest flex items-center justify-center gap-3 transition-all shadow-xl shadow-slate-900/20 text-[10px]"
                    >
                        View All Clients
